...
## Novetats 2026-09-09
- **Universos amb nom i tret propi** (`js/themes.js` `UNIVERSES`): després d'Univers (900) cada 400 roques: Aeon (l'òrbita respira), Kaal (dues roques en òrbita), Vesper (el punt verd parpelleja), Ilun (roques especials ×2), Sarme (gir ×1,3), Oriel (punt verd tremola), Nuvo (l'òrbita canvia de sentit cada 12 roques), Zairo (roques petites i ràpides). Es repeteixen com a Aeon II, Kaal II… Els trets s'apliquen a `game.mods` sota qualsevol millora (`_baseMods`, `_applyUniverse`).
- **Sortida des d'una etapa assolida**: fila de xips al menú (`#start-row`, `store.startStage`) i botó "Torna a sortir des de…" a la fi de partida. `game.reset({ startScore })` prefarceix un planeta equilibrat de la mida que té una partida real (`_prefill`). El repte diari sempre des de zero.
- **Equilibri**: el límit ja no baixa del 9,5% (abans 5%), perquè la roca normal més grossa posada al punt verd d'un planeta equilibrat no el trenqui. Roca pesada: 75% de mida i doble densitat. "Roques grans": més grans però amb el mateix pes. Amb punteria perfecta simulada es passa de morir a 60–150 roques a arribar a 1.000+.
- Anells i cinturons només bloquegen quan ja són al seu lloc i al radi on es dibuixen; la captura per gravetat es mesura des del punt de llançament; les roques de la pluja de meteorits ja arriben al planeta.
- S'ha tret el diàmetre en km.
